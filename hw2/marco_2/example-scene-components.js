...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 
    ...
    'construct'( context )
      ...
      { 
        controls.add( "w", this, function() { walk = 1; } ); controls.add( "w", this, function() { walk =  0; }, {'type':'keyup'});
        controls.add( "Space", this, function() { jump = 1;} ); 
    ...
    'display'( graphics_state )
      { 
        ...
        graphics_state.lights = [ new Light( vec4(  30,  30,  34, 1 ), Color( 0, .4, 0, 1 ), 100000 ),      // Arguments to construct a Light(): Light source position or 
            ...
            {
              if((i>=0&&i<=20)||(i>=23&&i<=33)||(i>=41&&i<=52)||(i>=54&&i<=61)||(i>=64))
                block_height[i] = 0;
            ...
            if(vy <0 && Math.abs(bally-block_height[get_z_unit(ballz)]-2) < 0.1 )
            {
              vy = 0;
              jump=0;
              ttmp=0;
            ...
            graphics_state.camera_transform = lookAt([0, 8, ballz+8], [0,bally,ballz], [0,1,0]);
