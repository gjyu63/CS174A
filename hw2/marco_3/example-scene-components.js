...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 
    ...
    'construct'( context )
      ...
      { 
        controls.add( "p", this, function() { walk = 1; } ); controls.add( "p", this, function() { walk =  0; }, {'type':'keyup'});
        controls.add( "l", this, function() { walk = -1; } ); controls.add( "l", this, function() { walk =  0; }, {'type':'keyup'});
    ...
    'display'( graphics_state )
      { 
        ...
        graphics_state.lights = [ new Light( vec4(  30,  30,  34, 1 ), Color( 0, .4, 0, 1 ), 100000 ),      // Arguments to construct a Light(): Light source position or 
            ...
            var get_z_unit = function(ballz){return Math.floor(Math.abs(ballz/2));}
            ...
            //   fly = 1;
            // }
            // // if(bally-block_height[get_z_unit(ballz)]+2 > 0)
            // // {
            // //   fly = 1;
            // // }
            // if(vy<0&&bally-block_height[get_z_unit(ballz)]-2 < 0 )
            // {
            //   fly = 0;
            ...
            //   vy = 0.5;
            // }
            // if(bally +1 - 2*block_height[get_z_unit(ballz)]>0)
            // {
            //   fly = 1;
            // }
            // if(fly == 1)
            ...
            if(bally-1-2*block_height[get_z_unit(ballz)] <=0 && block_height[get_z_unit(ballz)] != -1)
            {
              fly = 0;
              bally = 2*block_height[get_z_unit(ballz)] + 2;
              vy = 0;
            ...
            graphics_state.camera_transform = lookAt([20, 8, ballz+8], [0,bally,ballz], [0,1,0]);
