      ...
      { 
        ...
        graphics_state.lights = [ new Light( vec4(  30,  30,  34, 1 ), Color( 0, .4, 0, 1 ), 100000 ),      // Arguments to construct a Light(): Light source position or 
            ...
            if(scene_controller == 0)
            {
              ...
              this.  shapes.debug_text.set_string( "Beta Version 0.1.4");
              this.  shapes.debug_text.draw( this.graphics_state, txt_mt, this.text_material );  // Draw some UI text
              if( tt > 0.1)
              {
                txt_mt = mult(txt_mt, translation(0, -10, 0));
                if(Math.floor(tt)%3 == 0)
                  this.  shapes.debug_text.set_string( "Loading");
                if(Math.floor(tt)%3 == 1)
                  this.  shapes.debug_text.set_string( "Loading.");
                else if(Math.floor(tt)%3 == 2)
                  this.  shapes.debug_text.set_string( "Loading..");
                else
                  this.  shapes.debug_text.set_string( "Loading...");
            ...
            var counters = get_z_unit(ballz);
	          document.getElementById("counter").innerHTML = "Counter: " + counters;
      ...
      'draw_indicator'(model_transform, graphics_state, t)
      {
        ...
        this.shapes.pyramid.draw(graphics_state, model_transform, this.yellow_clay);
      },
