...
Declare_Any_Class( "Text_Line", // Draws a rectangle textured with images of ASCII characters textured over each quad, spelling out a string.
  ...
  Declare_Any_Class( "Semi_Sphere",         // With lattitude / longitude divisions; this means singularities are at 
  { populate( rows, columns )             // the mesh's top and bottom.  Subdivision_Sphere is a better alternative.
      { var circle_points = [];
        for( var i = 0; i <= rows/2; i++ )   circle_points.push( vec3( -Math.sin( i/rows * Math.PI ), 0, Math.cos( i/rows * Math.PI ) ) );
        
        Surface_Of_Revolution.prototype.insert_transformed_copy_into( this, [ rows, columns, circle_points ] );     
      } }, Shape ) 

Declare_Any_Class( "Pentahedron",
{ 'populate'()                  // vector, and texture-space coordinate.
                  {
                  this.positions.push(vec3(0,2,0), vec3(-1,0,-1), vec3(1,0,-1), vec3(0,2,0), vec3(-1,0,-1), vec3(-1,0,1), vec3(0,2,0), vec3(-1,0,1), vec3(1,0,1), vec3(0,2,0), vec3(1,0,1), vec3(1,0,-1), vec3(-1,0,-1), vec3(-1,0,1), vec3(1,0,-1), vec3(1,0,-1), vec3(1,0,1), vec3(-1,0,1));
                              
                                        
                  this.normals.push(vec3(0,-2,4),vec3(0,-2,4),vec3(0,-2,4), vec3(-4,2,0),vec3(-4,2,0),vec3(-4,2,0), vec3(0,2,4),vec3(0,2,4),vec3(0,2,4), vec3(4,2,-8),vec3(4,2,-8),vec3(4,2,-8), vec3(0,1,0),vec3(0,1,0),vec3(0,1,0),  vec3(0,1,0), vec3(0,1,0), vec3(0,1,0));

                  this.texture_coords.push(vec2(.33,0), vec2(0,1), vec2(.33,1),
                                           vec2(.33,0),vec2(0,1), vec2(.33,1), vec3(.66,0), vec3(.33,1), vec3(.66,1), vec2(.33,0), vec2(0,1), vec2(.33,1),
                                           vec2(.33,0),vec2(0,1), vec2(.33,1), vec3(.66,0), vec3(.33,1), vec3(.66,1) );
