...
Declare_Any_Class( "Ball",              // The following data members of a ball are filled in for you in Ray_Tracer::parse_line():
  ...
  //        return a dummy color if the intersection tests positiv.  This will show the spheres' outlines, giving early proof that you did intersect() correctly.
        ...
        var s = mult(this.inverse_trans,ray.origin), c = mult(this.inverse_trans, ray.dir);
        var delta = dot(s,c)*dot(s,c) - Math.abs(length(c)*length(c))*(Math.abs(length(s)*length(s)) - 1);
        if (delta < 0) return existing_intersection;  //guarnteed no intersection
        var t = 0, t2 = 0;
        if( delta == 0.0)
        {
          t = -dot(s,c)/(length(c)*length(c));
          ...
          else //touched
          {
            ...
            existing_intersection.normal = mult(transpose(this.inverse_trans), (add(s, scale_vec(t, c))));
          }
        }
        if(delta > 0)
        {
          t = -dot(s,c)/(length(c)*length(c)) + Math.sqrt(Math.pow(dot(s,c), 2) - length(c)*length(c)*(length(s)*length(s) - 1) )/(length(c)*length(c));
          t2 = -dot(s,c)/(length(c)*length(c)) - Math.sqrt(Math.pow(dot(s,c), 2) - length(c)*length(c)*(length(s)*length(s) - 1) )/(length(c)*length(c));
          if( t < minimum_dist || t > t2 ) t = t2;
          if( t < minimum_dist || t >= existing_intersection.distance ) return;
          existing_intersection.ball = this;
          existing_intersection.distance = t;
          existing_intersection.normal = mult(transpose(this.inverse_trans), (add(s, scale_vec(t, c))));
        }
