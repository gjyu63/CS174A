...
Declare_Any_Class( "Ray_Tracer",
  { 'construct'( context )
    ...
    //        ray or a recursion.  Use the argument light_to_check when a recursive call to trace() is for computing a shadow ray.
        ...
        var pos = add(toVec3(ray.origin), scale_vec(closest_intersection.distance, toVec3(ray.dir)));

        //var vision = normalize(subtract(toVec3(ray.origin), pos));
        ...
        var vicky = mult_3_coeffs(toVec3(ray.dir), [-1,-1,-1]); //ray.dir is 4d
        ...
        for(var i = 0; i<this.lights.length; i++)
        {
          ...
          var light_ray = {origin: pos.concat(1), dir: subtract(toVec3(lightning.position), pos).concat(0)};
          var shadow = this.trace(light_ray, color_remaining, false, false); 
 
            /*if there is intersections, no contribution to the color*/
        ...
        var pixel_color = add(surface_color, mult_3_coeffs(subtract( [1,1,1], surface_color),      add( scale_vec(interestedBall.k_r, this.trace( reflected, mult_3_coeffs([interestedBall.k_r,interestedBall.k_r,interestedBall.k_r], color_remaining), false, true  )).slice(0,3)     , scale_vec(interestedBall.k_refract, this.trace( refracted, mult_3_coeffs([interestedBall.k_refract,interestedBall.k_refract,interestedBall.k_refract], color_remaining), false, true  )).slice(0,3)      )          ));
      ...
      { graphics_state.lights = this.lights;
        ...
        { var y = this.scanline++;
          ...
          { var ray = { origin: mult_vec( camera_inv, vec4(0, 0, 0, 1) ), dir: mult_vec( camera_inv, this.get_dir( x, y ) ) };   // Apply camera
            this.set_color( x, y, this.trace( ray, [1,1,1], true, false ) );                                    // ******** Trace a single ray *********
