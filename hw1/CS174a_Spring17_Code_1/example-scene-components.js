...
Declare_Any_Class( "Example_Animation",  // An example of a Scene_Component that our class Canvas_Manager can manage.  This one draws the scene's 3D shapes.
  { 'construct'( context )
      { var shapes = { 'sphere'        : new Grid_Sphere(20, 20),                               // At the beginning of our program, instantiate all shapes we plan to use,
                      'box': new Cube()
    ...
    'display'( graphics_state )
      { 
        ...
        **********************************/                                     // From here on down it's just some example shapes drawn 
        ...
        var gj_mv = translation(0.8*t,0.5*t*Math.sin(t), 0);
        ...
        this.shapes.sphere.draw(graphics_state, mult(model_transform,gj_mv), this.greyPlastic);
        //box
        model_transform = mult(model_transform, translation(-3, 0, 0));
       // model_transform = mult(model_transform, scale(2,1,1));
        this.shapes.box.draw(graphics_state, mult(mult(model_transform, gj_mv), scale(2, 1,1)), this.greyPlastic); 
