...
{
  x.shapes.box.draw(graphics_state, mult(model_transform, scale(2,1,1)), x.yellow_clay);
}

function draw_sphere(model_transform, x, graphics_state)
{
  x.shapes.ball.draw(graphics_state, model_transform, x.brown_clay);
}

function draw_bee(model_transform, x, graphics_state, t)
{
  draw_box(mult(model_transform, translation(-1, 0, 0)), x, graphics_state);
  draw_sphere(mult(model_transform, translation(2,0,0)), x, graphics_state);
  //wing rotation matrix
  var rotwr = rotation(70*Math.sin(t), [1,0,0]);
  var rotwl = rotation(70*Math.sin(-t), [1,0,0]);

  //right wing
  draw_right_wings(mult(mult(model_transform, translation(0,0,1)), rotwr), x, graphics_state);
  //left wing
  draw_left_wings(mult(mult(model_transform, translation(0,0,-1)), rotwl), x, graphics_state);

}

function draw_right_wings(model_transform, x, graphics_state)
{
  model_transform = mult(mult(model_transform, translation(0,0,2)), scale(1,0.1,2));
  x.shapes.box.draw(graphics_state, model_transform, x.brown_clay);
}

function draw_left_wings(model_transform, x, graphics_state)
{
  model_transform = mult(mult(model_transform, translation(0,0,-2)), scale(1,0.1,2));
  x.shapes.box.draw(graphics_state, model_transform, x.brown_clay);
}
  ...
  { 'construct'( context )
    ...
    'display'( graphics_state )
      { 
        ...
        draw_bee(mult(mult(mv_x,model_transform), mv_y), this, graphics_state, t);
